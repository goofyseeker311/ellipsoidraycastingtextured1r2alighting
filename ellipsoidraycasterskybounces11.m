...
wimagefiles={"physical-world-map.jpg","Fondvulcain.jpg","world116cE.jpg", "galaxy2.jpg", "skygalaxy-hdri-114148912.jpg", "voie-lactee-hd-eso-l3.jpg"}; wimagescount=size(wimagefiles,2); wimages={}; wimagesizes=zeros(wimagescount,3);
for u=1:wimagescount; wimages{u}=double(imread(wimagefiles{u}))/255; wimagesizes(u,:)=size(wimages{u}); endfor;
lightvalue=1; pointlightscount = 5; pointlights = [100*(rand(pointlightscount,3)-0.5) 500*randi(255,pointlightscount,3)/255 -2*ones(pointlightscount,1)]; pointlightscount+=1; pointlights=[pointlights; [100 100 100 1 1 1 0]];
nspheres = 100; scenespheres = [100*(rand(nspheres,3)-0.5) 6*rand(nspheres,1)+4 2*rand(nspheres,3)+0.5 randi(255,nspheres,3)/255 randi(wimagescount,nspheres,1)];
...
for u=1:nspheres; a=scenespheres(u,4);ax=scenespheres(u,5);ay=scenespheres(u,6);az=scenespheres(u,7); x=scenespheres(u,1);y=scenespheres(u,2);z=scenespheres(u,3); surf((a/ax)*X+x,(a/ay)*Y+y,(a/az)*Z+z); endfor;
plot3(kx,ky,kz,'y.'); plot3(pointlights(:,1),pointlights(:,2),pointlights(:,3),'rx'); hold off;

kl=zeros(raycount,3); for v=1:pointlightscount; printf("v/plcount: %i/%i\n",v,pointlightscount); pla=(ones(kcicount,1)*pointlights(v,1:3)); minlightdist=1.1; pllc=pointlights(v,7);
plv=pla.-knc; pld=sqrt(sum(plv.^2,2)); pld3=pld*ones(1,3); plnv=plv./pld3; lv=[knc.+(minlightdist.*plnv) plnv]; [ck, ckn] = raysphereintersection(scenespheres(:,1:7), lv); ckl=(ck(:,2)==(nspheres-pointlightscount+v));
cki=find(ckl); ckir=find(!ckl); ckd=ck(cki,1); ckdr=ckd.^pllc; ckicount=size(cki,1);ckircount=size(ckir,1); ckni=ckn(cki,:);cknir=ckn(ckir,:); kcni=kci(cki);kcnir=kci(ckir); pld3n=pld3(cki,:);
plc=ones(ckicount,1)*pointlights(v,4:6); kl(kcni,:).+=(lightvalue.*ckdr.*plc);kl(kcnir,:)=ones(ckircount,1)*[0.2 0.2 0.2]; figure(1); hold on; plot3(ckni(:,1),ckni(:,2),ckni(:,3),'r.'); hold off; endfor;
